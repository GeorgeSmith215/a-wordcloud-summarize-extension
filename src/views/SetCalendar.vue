  ...
  <div class="setCalendar">
    ...
    <div class="todos">
      ...
      <div class="btnWarp">
        ...
        <div class="primary btn" @click="getPageContent">获取</div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      nextTick(() => {
        ...
        if (request.action === 'getPageSource') {
          pageContent.innerHTML = request.source
        }
      });
      })
    });
    ...
    const getPageContent = () => {
      // 假设 pageSource 包含了整个页面的源代码
      var pageSource = document.documentElement.outerHTML;

      // 创建一个虚拟的HTML元素，将源代码插入其中
      var virtualElement = document.createElement('html');
      virtualElement.innerHTML = pageSource;

      // 获取 <body> 元素
      var bodyElement = virtualElement.querySelector('body');
      if (bodyElement) {
        var bodyContent = bodyElement.innerHTML;
